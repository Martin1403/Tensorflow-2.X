...
def transcribe(path, model, scorer, audio_folder, metadata_path):
    ...
    from .utilities import filter_text, read_text, bold_text, bold_line
    print("\n")
    bold_line(t="START", n=40)
    text = filter_text(read_text(path))
    bold_text("SPEAK", text)
    for frame in frames:
        if frame is not None:
            ...
            audio_name = f"MV-001-{last_number}"
